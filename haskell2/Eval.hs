...
malEVAL env (MalList ((MalSymbol "let*"):args)) = letstar env args
...
letstar :: MalEnv -> [MalType] -> (MalEnv, MalType)
letstar env ((MalList bindings):result:[]) = letstar' env bindings result
letstar env ((MalVector bindings):result:[]) = letstar' env bindings result
...
multipledefs env (MalSymbol key:z:xs) =
